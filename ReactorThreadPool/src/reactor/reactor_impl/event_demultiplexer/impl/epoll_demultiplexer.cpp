    ...
    void EpollDemultiplexer::WaitEvents(std::map<handle_t, std::shared_ptr<IEventHandler>> *handlers)
    {
        ...
        if (num > 0)
        {
            for (size_t i = 0; i < num; i++)
            {
                ...
                {
                    auto error_task = std::bind(&IEventHandler::HandleError, (*handlers)[fd]);
                    thread_pool_->enqueue(error_task);
                ...
                else
                {
                    ...
                    {
                        auto read_task = std::bind(&IEventHandler::HandleRead, (*handlers)[fd]);
                        thread_pool_->enqueue(read_task);
                    }
